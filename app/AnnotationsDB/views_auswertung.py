	...
	if aTagEbene > 0:
		...
		if getXls:
			...
			for obj in aAuswertungen:
				...
				ws.write(row_num, 14, xls_max_chars(obj['aTokensText']), font_style)
				ws.write(row_num, 15, xls_max_chars(obj['aTokens']), font_style)
				if obj['aAntTags']:
					ws.write(row_num, 16, xls_max_chars(obj['aAntTags']['t']), font_style)
				dg = 0
				for nATT in nAntTagsTitle:
					if nATT['i'] in obj['nAntTags']:
						ws.write(row_num, 17 + dg, xls_max_chars(obj['nAntTags'][nATT['i']]['t']), font_style)
					dg += 1
			if html:
				wb.save(response)
				return ['xls', response]
